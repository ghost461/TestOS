...
// 屏幕输入光标的移动
// 在本模块内，cursor_x和cursor_y这两个变量指明了逻辑上的当前输出位置，
// 但是并没有实际上移动硬件的显示“光标”
static void move_cursor()
{
	...
	// VGA内部寄存器多达300多个，显然无法一一映射到I/O端口的地址空间。
	// 对此VGA控制器的解决方案是，将一个端口作为内部寄存器的索引：0x3D4，
	// 再通过0x3D5端口来设置相应寄存器的值
	// 在这里用到的两个内部寄存器的编号为14与15，分别表示光标位置的高8位与低8位
	outb(0x3D4, 14);                          // 告诉VGA 我们要设置光标的高字节
	outb(0x3D5, cursorLocation >> 8);         // 发表高8位
	outb(0x3D4, 15);                          // 告诉VGA我们要设置光标的低字节
	outb(0x3D5, cursorLocation);              // 发表低8位
}
...
// 屏幕滚动显示
// 将后24行的数据全部向上移动一行，最后一行清空
static void scroll()
{
	// sttribute_byte 被构造出一个黑底白字的描述格式
	uint8_t attribute_byte = (0 << 4) | (15 & 0x0F);
	uint16_t blank = 0x20 | (attribute_byte << 8);
